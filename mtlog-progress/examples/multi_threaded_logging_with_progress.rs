...
fn main() {
    ...
    let guard = logger_config()
        .with_log_file("/tmp/log_with_progress.log")
        .unwrap()
        .init_global();
    log::info!("Hello, Top !");
    std::thread::spawn(move || {
        let pb = LogProgressBar::new(100, "Thread1").with_min_timestep_ms(5.);
        for _ in 0..50 {
            pb.inc(1);
    ...
    let handle = std::thread::spawn(move || {
        let pb = LogProgressBar::new(100, "Thread2").with_min_timestep_ms(5.);
        for _ in 0..100 {
            pb.inc(1);
            std::thread::sleep(std::time::Duration::from_millis(1));
        }
    ...
    handle.join().unwrap();
    drop(guard); // ensure all logs are flushed
